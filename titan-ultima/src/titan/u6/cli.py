...
# ============================================================================
# CMD_* IMPLEMENTATION FUNCTIONS — Z-ORDER AUDIT
# ============================================================================

def _zorder_audit_helpers(tileflags: U6TileFlags):
    """Same anchor/overflow/layer-tier rules as ``cmd_map_render``'s ``--objects`` compositing.

    Hand-kept copy of that logic (see ``cmd_map_render``'s own CARPET_OBJ_N /
    MISFLAGGED_FOREGROUND_TILES / is_background / layer -- this mirrors them).
    If that function's layering rules change, update this to match or the
    audit drifts out of sync with what actually renders.
    """
    CARPET_OBJ_N = 303
    SECRET_DOOR_OBJ_N = 334
    DOORWAY_OBJ_N = 301
    ...
    # obj_n=339 dead body (frames 0-8, tiles 1262-1270; obj_n=338's own frame 9,
    # tile 1271 "pile of bones", stays unlisted/plain). obj_n=341 dead gargoyle
    # (frames 0-3, tiles 1276-1279) stays unlisted/plain; its rival club (545)
    # is listed instead. obj_n=17/18 cloth/leather armour (528/529) stay
    # unlisted/plain; their rivals (mug 645, nightshade mushroom 581, candle
    # 647) are listed instead. Cleaver/knife (637-639) beat blood (1259-1261).
    # Magic bow (565) beats part of a map (1757, dungeon2 loot).
    MISFLAGGED_FOREGROUND_TILES = {
        1126, 1127, 1132, 1133,  # cookfire
        1262, 1263, 1264, 1265, 1266, 1267, 1268, 1269, 1270,  # dead body
        545,  # club
        ...
        565,  # magic bow
        1240, 1241, 1242, 1247,  # sign plaques that beat their own signpost (1246)
    }
...
def _zorder_audit_scan(obj_iter, zone_label, tiles, tileflags, basetile, anim, names, is_background, layer,
    ...
    for pos, entries in by_pos.items():
        ...
        for loser in tied_at_top[1:]:
            if (
                loser["opac"] >= min_loser_opacity
                and winner["opac"] >= min_winner_opacity
...
def cmd_map_audit_zorder(args: SimpleNamespace) -> int:
    """Whole-map audit for unresolved object z-order ties in ``map-render --objects``."""
    gamedir = _require_gamedir(args.gamedir)
    for name in ("BASETILE", "TILEFLAG", "ANIMDATA", "LOOK.LZD", "LZOBJBLK", "LZDNGBLK"):
        if not os.path.isfile(os.path.join(gamedir, name)):
    ...
    tileflags = U6TileFlags.from_file(os.path.join(gamedir, "TILEFLAG"))
    tiles = U6Tiles.from_directory(gamedir)
    anim = U6AnimData.from_file(os.path.join(gamedir, "ANIMDATA"))
    names = U6ObjectNames.from_file(os.path.join(gamedir, "LOOK.LZD"))
    is_background, layer = _zorder_audit_helpers(tileflags)
    ...
    candidates = _zorder_audit_scan(world.iter_surface(), "surface", **kwargs)
    if args.dungeons:
        for level in range(5):
            candidates += _zorder_audit_scan(world.iter_dungeon(level), f"dungeon{level}", **kwargs)
...
@u6_app.command("map-audit-zorder")
def map_audit_zorder_cmd(
    gamedir: Annotated[Optional[str], typer.Option("-g", "--gamedir", help="U6 game directory")] = None,
    min_winner_opacity: Annotated[
        float,
        typer.Option(
            "--min-winner-opacity", help="Min opacity for a tied tile to count as a plausible occluding 'winner'"
        ),
    ] = 0.60,
    min_loser_opacity: Annotated[
        float,
        typer.Option(
            "--min-loser-opacity", help="Min opacity for a tied tile to count as a real, hideable 'loser'"
        ),
    ] = 0.10,
    limit: Annotated[int, typer.Option("--limit", help="Max rows to print (0 = unlimited)")] = 200,
    dungeons: Annotated[bool, typer.Option("--dungeons", help="Also scan all 5 dungeon levels")] = False,
) -> None:
    """Find candidate object z-order ties that map-render --objects could be compositing wrong.

    Reproduces map-render --objects's own anchor/overflow/layer-tier rules and
    flags every world coordinate where two or more objects land on the exact
    same final tier -- a coin-flip decided by file order, not a deliberate
    hierarchy. A reported pair is a candidate, not a confirmed bug -- cross-
    check against a real screenshot before trusting one.
    """
    raise SystemExit(cmd_map_audit_zorder(SimpleNamespace(
        gamedir=gamedir, min_winner_opacity=min_winner_opacity, min_loser_opacity=min_loser_opacity,
        limit=limit, dungeons=dungeons,
    )))
