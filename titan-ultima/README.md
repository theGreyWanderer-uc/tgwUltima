...
| Animation clips | Parse every `anim.flx` clip's authoring range/path, named transform tracks and typed events; optionally attach original names from Ghidra data; export registry-backed model/type/usecode candidates or one explicitly selected rigid-limb pose | `titan u9 animation-pose-export static/anim.flx 172 static/sappear.flx 3223 --time-ms 500 -o avatar_pose/` |
